...
public class StationDAOImpl implements StationDAO {
    ...
    @Override
    public List<ComboBoxStr> getStationObjectTypeComboBox(String query, String idType) throws SQLException {
        List<ComboBoxStr> comboBoxes = new ArrayList<>();
        if(idType== null || idType.equals("")){
                return comboBoxes;
        }
        String sql = "select s.station_id as id, s.station_code as code, s.station_name as name from stations s INNER join stations_object_type st on st.station_id = s.station_id where 1=1 ";
        if (query != null && !query.equals("")) {
            sql = sql + " and (UPPER(s.station_name) like ? or UPPER(s.station_code) like ?)";
        }
        sql = sql + " and st.object_type_id = ? and rownum < 100 and s.ISDEL = 0 and s.IS_ACTIVE = 1 order by s.station_code";
        try (Connection connection = ds.getConnection() ; PreparedStatement statement = connection.prepareStatement(sql);) {
            statement.setLong(1, Long.parseLong(idType));
            if (query != null && !query.equals("")) {
                statement.setString(2, "%" + query.toUpperCase() + "%");
