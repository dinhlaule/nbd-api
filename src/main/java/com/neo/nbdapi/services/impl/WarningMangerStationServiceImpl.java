...
public class WarningMangerStationServiceImpl implements WarningMangerStationService {
    ...
    public DefaultPaginationDTO getListWarningThresholdStation(DefaultRequestPagingVM defaultRequestPagingVM) throws SQLException, BusinessException {
        ...
        try (Connection connection = ds.getConnection()) {
            ...
            StringBuilder sql = new StringBuilder("select w.id, s.station_id, s.station_name, w.code, w.name, w.icon, w.created_at, w.description, w.content, w.color, w.suffixes_table from warning_manage_stations w inner join stations s on s.STATION_ID = w.STATION_ID inner join stations_object_type st on st.station_id  = s.station_id inner join object_type o on o.object_type_id  = st.object_type_id where s.isdel = 0 and s.IS_ACTIVE = 1 ");
            ...
            if (Strings.isNotEmpty(search)) {
                ...
                if (Strings.isNotEmpty(objectSearch.getType())) {
                    sql.append(" and o.object_type_id = ? ");
                    paramSearch.add(objectSearch.getType());
                }
